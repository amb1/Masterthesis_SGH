...
def map_building_use(building_type: str, config_path: str = "cfg/wfs/wfs_config.yml") -> str:
    """
...
def get_standard_prefix(building_type: str, config_path: str = "cfg/wfs/wfs_config.yml") -> str:
    """
...
def combine_building_standard(building_type: str, year: int, config_path: str = "cfg/wfs/wfs_config.yml") -> str:
    """
...
def shorten_id(value: str, max_length: int = 10) -> str:
    """
    Kürzt eine ID auf die angegebene Länge.
    Verwendet einen Hash für konsistente Kürzung.
    
    Args:
        value (str): Die zu kürzende ID
        max_length (int): Maximale Länge der gekürzten ID
    ...
    # Erstelle einen Hash der ID
    hash_object = hashlib.md5(value.encode())
    hash_hex = hash_object.hexdigest()
    
    # Kürze auf die gewünschte Länge
    return hash_hex[:max_length]

def transform_value(value: Any, transform_config: Dict[str, Any]) -> Any:
    """
    Führt eine Transformation auf einen Wert durch.
    
    Args:
        value: Der zu transformierende Wert
        transform_config: Konfiguration der Transformation
    ...
    transform_type = transform_config.get('function')
    
    if transform_type == 'shorten_id':
        max_length = transform_config.get('max_length', 10)
        return shorten_id(str(value), max_length)
        
    logger.warning(f"⚠️ Unbekannte Transformation: {transform_type}")
    return value 