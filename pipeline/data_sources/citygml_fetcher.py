...
class CityGMLBuildingProcessor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.ns = self.citygml_config.get('namespaces', {})
        self.output_formats = config.get('output', {}).get('formats', [])
        self.crs_config = config.get('processing', {}).get('crs', {})
    ...
    def process_file(self, citygml_path: str) -> bool:
        """Verarbeitet eine CityGML-Datei vollständig.
        ...
        try:
            # Extrahiere Gebäude
            buildings_gdf = self.extract_buildings(citygml_path)
            if buildings_gdf is None:
                return False
                
            # Transformiere CRS wenn konfiguriert
            if self.crs_config.get('output'):
                buildings_gdf = self.transform_crs(buildings_gdf)
                
            # Validiere Geometrien wenn konfiguriert
            if self.config.get('processing', {}).get('validation', {}).get('check_geometry', True):
                buildings_gdf = self.validate_geometries(buildings_gdf)
            
            # Exportiere in alle konfigurierten Formate
            for format_config in self.output_formats:
                ...
                if format_type == '3dtiles':
                    self.export_3dtiles(buildings_gdf, output_dir, format_config.get('options', {}))
                elif format_type == 'geojson':
                    self.export_geojson(buildings_gdf, output_dir / 'buildings.geojson', 
                                      format_config.get('options', {}))
                    
            self.logger.info(f"✅ Verarbeitung abgeschlossen für: {citygml_path}")
            return True
    ...
    def transform_crs(self, gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        """Transformiert das CRS des GeoDataFrames."""
        try:
            target_crs = self.crs_config.get('output')
            if target_crs and gdf.crs != target_crs:
                self.logger.info(f"🔄 Transformiere CRS von {gdf.crs} nach {target_crs}")
                gdf = gdf.to_crs(target_crs)
                self.logger.info("✅ CRS-Transformation abgeschlossen")
            return gdf
        except Exception as e:
            self.logger.warning(f"⚠️ Fehler bei CRS-Transformation: {str(e)}")
            return gdf

    def validate_geometries(self, gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        """Validiert und repariert Geometrien wenn nötig."""
        try:
            invalid_geoms = gdf[~gdf.geometry.is_valid]
            if not invalid_geoms.empty:
                self.logger.warning(f"⚠️ {len(invalid_geoms)} ungültige Geometrien gefunden")
                if self.config.get('processing', {}).get('validation', {}).get('fix_invalid', True):
                    gdf.geometry = gdf.geometry.buffer(0)
                    self.logger.info("✅ Geometrien repariert")
            return gdf
        except Exception as e:
            self.logger.warning(f"⚠️ Fehler bei Geometrie-Validierung: {str(e)}")
            return gdf

    def export_3dtiles(self, gdf: gpd.GeoDataFrame, output_dir: Path, options: Dict[str, Any]) -> None:
        """Exportiert Gebäude als 3D Tiles."""
        try:
            # Implementierung des 3D Tiles Exports hier
            self.logger.info(f"🔄 Exportiere 3D Tiles nach {output_dir}")
            # TODO: Implementiere 3D Tiles Export
            self.logger.info("✅ 3D Tiles Export abgeschlossen")
        except Exception as e:
            self.logger.error(f"❌ Fehler beim 3D Tiles Export: {str(e)}")

    def export_geojson(self, gdf: gpd.GeoDataFrame, output_path: Path, options: Dict[str, Any]) -> None:
        """Exportiert Gebäude als GeoJSON."""
        try:
            # Optionale Vereinfachung der Geometrien
            if options.get('simplify_tolerance'):
                gdf = gdf.copy()
                gdf.geometry = gdf.geometry.simplify(options['simplify_tolerance'])
                
            # Export
            self.logger.info(f"🔄 Exportiere GeoJSON nach {output_path}")
            gdf.to_file(output_path, driver='GeoJSON')
            self.logger.info("✅ GeoJSON Export abgeschlossen")
        except Exception as e:
            self.logger.error(f"❌ Fehler beim GeoJSON Export: {str(e)}")
...
def fetch_citygml_buildings(citygml_file: str, output_dir: str, config: Dict[str, Any]) -> Optional[gpd.GeoDataFrame]:
    ...
    Diese Funktion ist der Haupteinstiegspunkt für die CityGML-Verarbeitung.
    Sie initialisiert den Prozessor und führt die Verarbeitung durch.
    ...
    try:
        ...
        # Verarbeite Datei
        success = processor.process_file(citygml_file)
        
        if success:
            logger.info(f"✅ CityGML-Verarbeitung abgeschlossen: {citygml_file}")
            return processor.extract_buildings(citygml_file)
        else:
            logger.error("❌ CityGML-Verarbeitung fehlgeschlagen")
            return None
            
    except Exception as e:
        logger.error(f"❌ Fehler bei CityGML-Verarbeitung: {str(e)}")
        return None
